...
if (isset($_REQUEST["clicoupesquisa"]) == "1")
{
  ...
  $obj= new Database;
  $resultado = $obj->connect("select * from logs where COD_USUARIOS_SISTEMA like '%$pesquisa%' order by HORA_LOGS DESC");

  
}else{
  $obj= new Database;
  $resultado = $obj->connect("select * from logs order by HORA_LOGS DESC");
}
?>
    
<div class="ls-box ls-board-box">
  <header class="ls-info-header">
    <p class="ls-float-right ls-float-none-xs"> <button  data-ls-module="modal" data-action="lib/clean_logs.php?user_loged=<?php echo $_SESSION['login']; ?>"  data-content="
    ...
    <h2 class="ls-title-3">Registros de Acessos dos Usuários</h2>
  </header>
...
<!-- Pesquisa -->
<br>
   <form  method="post" action="logs_usuario.php?clicoupesquisa=1" class="ls-form ls-form-inline ls-float-right">
...
/*Contador Paginacao
  $sql = "select count(*) from logs_2 where TIPO_LOGS='AUTENTICACAO' or TIPO_LOGS='SAIDA DO SISTEMA'";
  $resultado=conecta($maquina,$usuario,$senha,$banco,$sql);
  while($linha=mysql_fetch_array($resultado)) { $contaregistros = $linha[0]; }

  $paginacao = $contaregistros / $registrosPorPagina;

  if($paginacao <1 ){
    $paginacao="1";
  }else {
    $paginacao=ceil($paginacao);
  }

End Contador Paginacao*/
